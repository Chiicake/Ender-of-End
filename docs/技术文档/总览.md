...
|  - input: clip + retrieval   |     |  - input: image/history+DSL  |
...
repo-root/
  ...
  src/
    game_agent_train/
      ...
      labeler/                     # VLM Labeler：clip -> plan_json（离线监督信号）
        run_labeler.py
        prompts.py
        postprocess.py
...
- **主要作用**：
  - 录制目标窗口游戏画面（5FPS）
  - 聚合原始输入事件到 200ms step
  - 输出动作字符串（含 6×33ms chunk）
- **输入**：游戏窗口
- **输出**：
  - `video/frames`（或视频文件）
  - `actions.txt`（每 200ms 的 action string）
...
- **输出**：
  - `planner_dataset`：`(clip + mid_step + retrieved_memory) -> plan_json`
  - `controller_dataset`：`(image/history + plan_dsl) -> action_string`

### 2.3 VLM Labeler（标注器）
- **技术栈**：Python（Linux）；基于api，输入端口号连接到远端部署的模型进行数据传输和收集
- **主要作用**：
  - 将短视频片段（clip）与当前 `mid_step` 转换为结构化 `plan_json`（含 DSL short_goal）
  - 产出训练监督信号（尤其用于 Planner 训练）
- **输入**：`clip + mid_step_id/text (+ constraints)`
...
- **主要作用**：
  - 微调 Qwen3-VL，使其在给定 `clip + mid_step + retrieved_memory` 时输出 `plan_json`
- **输入**：`planner_dataset`
...
1. 清洗与对齐：去掉异常段，保证 5FPS 与 200ms step 一致
2. 片段切分：为每个 mid_step 生成需要标注/训练的 clip
3. 调用 VLM Labeler：clip → plan_json（作为监督信号）
4. 生成：
