...
> - **Planner 低频规划 Mid Step 与生成 Short Goal（用 DSL 表达）**；**Planner 每次读取 MCP：最近 30–60 秒事件 + Top-K 语义相关历史尝试/事件**
> - **Controller 高频执行 Short Goal → 输出 action-only**（500ms，内部 6×83ms）
...
- 动作细分：每次输出包含 **6 组按键**（每组代表 **83ms**）

### 0.2 动作字符串协议（Controller 输出必须严格符合）
```
<|action_start|>dx dy dz ; group1 ; group2 ; group3 ; group4 ; group5 ; group6 <|action_end|>
```
- `dx dy dz`：鼠标相对位移 x、y 与滚轮 z
- `group1..group6`：每组为 83ms 内按下的键集合（空组允许，但必须保留分号位置）
...
- **VLM Labeler（离线）**：对短视频片段生成结构化标签（DSL short_goal + done_evidence 等）
- **Planner（低频）**：运行时生成计划与 short_goal（DSL），并写入 MCP
- **Controller（高频）**：执行 short_goal（DSL）并输出 action-only
- （暂不做）**EventGate（分级触发器）**：L0/L1 持续运行触发；L2 重感知仅在触发时运行
- **MCP（外置记忆/控制面板，增强版）**：Timeline Log + Retrieval
- **Runtime Orchestrator**：2FPS 调度、83ms 注入、门控与回退、日志写入、span 结束判定
- **Evaluation Suite**：离线指标 + 在线基准场景 + 回归测试

### 1.3 运行时闭环（每 500ms）
1) 捕获当前帧 `image_t`
2) （暂不做）`EventGate(L0/L1, image_t, history, last_actions) -> triggers/events`
...
9) 按 83ms tick 依次执行 6 个 group（键鼠注入）
...
> **模型不负责从短视频猜长期语义**。长期/中期由外置流程提供，模型只在“已知当前步骤”的条件下生成并执行短期目标。

### 2.1 层级定义（区分 Mid Goal 与 Mid Step）
- **Long Goal（长期任务）**：会话级目标（例如“完成日常”）
- **Mid Goal（中期任务/流程块）**：可复用的大块流程（例如“刷副本、跑图、收菜”）
- **Mid Step（中期步骤）**：Mid Goal 内可判定完成的步骤序列，由 Planner 生成
- **Short Goal（短期目标）**：给 Controller 的 1–10 秒可执行意图；在本方案中用 **DSL** 表达，通常在同一 Mid Step 内多次迭代直到完成

### 2.2 层级示例
- **Long Goal**：完成日常任务
- **Mid Goals**：刷副本（`run_dungeon`）、跑图（`world_travel`）、收基建（`harvest_farming`）
- **Mid Steps（举例：跑图到基地接任务的一段）**：
  - `mid_step_id = go_gate_area`：前往基地附近 NPC 区域（证据：到达区域/导航 marker 变化等）
...
2) **构造 Controller 样本（Span-aligned）**：
...
3) **过滤与黄金集**：对 uncertainty=high 丢弃/复核；每个 mid_step 固定黄金集
4) **失败恢复样本回流**：从 attempt_log 挖掘连续失败→fallback→成功的片段，增强 Planner 训练

### 3.4 （暂不做）Retrieval 字段构造
...
## 5. Planner（低频：long/mid goal + recent/summary clip + retrieval → attempt + mid_step + short_goal_dsl，训练与运行统一）

### 5.1 Planner 输入
- `long_goal`
- `mid_goal`
- `recent_clip`（4 秒 8 帧）
- `summary_clip`（60 秒历史，每 2 秒采 1 帧）
- `mid_step_text`（当前的mid_step，为上一步planner的输出值，可能为空）
- `prev_attempt` 之前对于任务执行的思考，为上一步planner的输出值，可能为空
- `prev_short_goal_dsl`（用于平滑）

（暂不做）
- `events`（EventGate 输出，可选）
- `state_summary`（可选：L1/L2 汇总结构化状态）
- `recent_window_events`：可选，最近 30–60 秒事件/尝试摘要
- `topK_related`：可选，与当前 mid_step 语义相关的 K 条历史尝试/事件（失败原因、有效策略）

### 5.2 Planner 输出
- `plan_id`
- `next_mid_step` 若当前mid step结束则进行下一个，否则保持当前不变
- `short_goal_dsl`
- `attempt` 对于当前的任务执行的思考
- `horizon_steps` 指规划中本short goal剩余的帧数，如果到期则会自动调用Planner
- `done_evidence` event检测到done evidence，检测到会自动调用Planner
...
### 8.1 MCP 写入
- （暂不做）`event_log`：来自 EventGate L0/L1/L2 的事件（采样/去重）
- （暂不做）`state_summary`：L2 OCR/强检测汇总的结构化状态（低频写入）
- `short_goal_dsl`：Planner 输出（含 plan_id、horizon）
- `mid_step_transition`：mid_step 前进/回退记录
- `attempt_log`：每次 plan span 结束写入 outcome（成功/失败/超时、失败原因、证据）

### 8.2 MCP 读取
- Controller：读取 `short_goal_dsl + plan_id + horizon`
- Planner：每次规划读取：
...
- `image_t`（2FPS）
- history buffer
- MCP 状态（short_goal + plan_id/horizon、recent summary）
...
7) 若 `need_plan`：
   ...
   - 异步调 Planner（Planner-LoRA）生成 new short_goal（含新 plan_id）
...
- **Planner-LoRA（TaskAdapter）**：低频生成 short_goal_dsl JSON（含 plan_id）
...
- 目标：为每个 mid_step 产出一致的 short_goal_dsl
- 输出：Planner 训练集 + 黄金集
...
- [ ] Labeler（离线 VLM 输出 short_goal_dsl JSON
- [ ] Dataset Builder：
