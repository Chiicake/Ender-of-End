...
- **Planner 训练集（Retrieval-aware）**：
  ...
  - target：`short_goal JSON（plan_id 由 Builder 注入）`
...
5) **离线 VLM Labeler 标注 short_goal**：输出结构化 JSON + uncertainty
...
- `lookahead_clip`：60 秒未来帧，每 2 秒采 1 帧（约 30 帧，仅标注用）
- `mid_goal_id`（可选但建议提供）
...
>
> 备注：若输出缺失字段，Builder 按配置填充默认值。

```json
{
  "mid_step_id": "mid_XX",
  "next_mid_step_text": "收取基建产出",
...
- 任务说明：
  - 输入包含：`mid_step_id/mid_step_text`（当前步骤）、`recent_clip`（当前到过去 4 秒）、`summary_clip`（过去 60 秒摘要）、`lookahead_clip`（未来 60 秒摘要，仅标注用）、`constraints`、以及枚举表。
  - 输出字段含义：
...
- lookahead_clip: （30 帧，60 秒未来每 2 秒采 1 帧，仅标注用）
