...
> 目标：用纯视觉输入实现 3D 动作网游自动化（5FPS 决策、33ms tick 执行）。
>
...
> - **Controller 高频执行 Short Goal → 输出 action-only**（200ms，内部 6×33ms）
> - **MCP 增强为 Timeline Log + Retrieval**：写入 event_log/state_summary/attempt_log/step 迁移；支持 recent window 与 topK 检索
...
### 0.1 采样与执行
- 决策频率：**5 FPS**，每 **200ms** 生成一次动作
- 动作细分：每次输出包含 **6 组按键**（每组代表 **33ms**）
...
- `group1..group6`：每组为 33ms 内按下的键集合（空组允许，但必须保留分号位置）
...
|  | 5 FPS     |                   | L0/L1/L2 cascade|                  | + retrieval inputs  |  |
|  +-----------+                   +--------+--------+                  | -> short_goal_dsl   |  |
|                                           |                           | -> plan_id/span cfg |  |
|                                           | write logs/updates        +----------+---------+   |
|                                           v                                      |             |
...
|                                           v                                                    |
|                                   +-------+--------+                                           |
|                                   | Input Injector | 6x33ms tick                               |
|                                   +----------------+                                           |
+------------------------------------------------------------------------------------------------+
```

### 1.2 组件列表
- **Collector**：采集视频 + RawInput，对齐到 200ms step，生成 action string
...
- **VLM Labeler（离线）**：对短视频片段生成结构化标签（DSL short_goal + done_evidence 等，含 plan_id/terminate_on）
...
- **Runtime Orchestrator**：5FPS 调度、33ms 注入、门控与回退、日志写入、span 结束判定
...
5) 若 `need_plan=true`：
   ...
   - 调用 `Planner(recent_clip + mid_step + retrieved_memory) -> short_goal_dsl + done_evidence + plan_id + span_cfg + updates`
...
9) 按 33ms tick 依次执行 6 个 group（键鼠注入）
...
- episode：`{image_t}`（5FPS）
- 对齐动作：`{action_t}`（200ms action string）
...
- **Planner 训练集（Retrieval-aware）**：
  - input：`recent_clip + mid_goal_id + mid_step_id + mid_step_text + retrieved_memory`
  - target：`short_goal JSON（含 plan_id/terminate_on）`
...
3) **mid_goal/mid_step 对齐**：
   - 最稳：脚本给出区间；
   - 半自动：UI 证据 + 规则切段 + 抽检。
4) **片段抽取（Planner/Labeler）**：`recent_clip=[t-19..t]`（20 帧≈4 秒）
5) **离线 VLM Labeler 标注 short_goal（含 plan_id/terminate_on）**：输出结构化 JSON + uncertainty
6) **构造 Planner 样本（Retrieval-aware，对齐点 1）**：
   - 从日志自动生成 `retrieved_memory={recent_window_events, topK_related}`（§3.4）
   - 将 retrieved_memory 与 clip/mid_step 一起作为输入；target 为 Labeler 输出 JSON
...
### 3.4 Retrieval 字段构造（Planner 训练对齐点 1，必须落地）
...
**Span 定义**
- 每次 Labeler/Planner 产生一个 short_goal，分配唯一 `plan_id`
...
## 4. VLM Labeler（离线标注器：clip → DSL short_goal，加入 plan_id/terminate_on）

### 4.1 VLM Labeler 输入
- `recent_clip`：20 帧（4 秒；可调 2–6 秒）
- `mid_goal_id`（可选但建议提供）
...
- 你必须只输出 JSON，严格符合给定 schema，不得输出解释文字。
...
## 5. Planner（低频：mid_step + clip + retrieval → short_goal_dsl，训练与运行统一）
...
**System**
- 你是短期目标生成器。根据当前步骤目标（Mid Step）、视频片段，以及 MCP 提供的历史摘要（recent + topK），输出 short_goal（DSL）和完成证据。
- 只输出 JSON，严格 schema。
...
**System**
- 你是实时游戏控制器。根据当前画面与 short_goal（DSL），输出未来 200ms 的键鼠动作。
...
- `retrieve(query, k=5, filters={mid_step_id?}) -> topK_items`
...
### Orchestrator 核心流程（每 200ms）
1) capture `image_t`
2) EventGate L0/L1：得到 `need_L2 / events_L1 / stuck_suspect`
3) 若 `need_L2`：运行 L2 → `state_summary/events_L2`
...
7) 若 `need_plan`：
   ...
   - 调 Planner（Planner-LoRA）生成 new short_goal（含新 plan_id/terminate_on）
...
- 目标：为每个 mid_step 产出一致的 short_goal_dsl（含 plan_id/terminate_on）
- 输出：Planner 训练集 + 黄金集
...
- [ ] Orchestrator（200ms loop + 33ms tick）
...
- [ ] Labeler（离线 VLM 输出 short_goal_dsl JSON，含 plan_id/terminate_on）
- [ ] Dataset Builder：
